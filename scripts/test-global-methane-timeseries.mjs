...
const source = "data/knowledge/gwl_climate_methane_global_v0.1.json";
const seriesId = "global_methane_noaa_annual_1984_2025";
...
if (!series) throw new Error("Globale Methanreihe fehlt.");
...
if (payload.presentation?.gwlTimeSeriesDisplay !== "link_only"
  ...
  throw new Error("Die Methankurve darf im GWL nicht dargestellt werden.");
}
...
if (series.dataNature !== "observed" || series.points.length !== 42
  || series.points[0].year !== 1984 || series.points.at(-1).year !== 2025) {
  throw new Error("NOAA-Methanbeobachtung ist unvollständig.");
}
if (Math.abs(series.points[0].value - 1644.84) > 0.01 || Math.abs(series.points.at(-1).value - 1935.94) > 0.01) {
  throw new Error("NOAA-Methanwerte stimmen nicht mit der Quelldatei überein.");
}
const historical = series.historicalSegments?.[0];
if (!historical || historical.points.length < 20 || historical.points[0].year > 1750 || historical.points.at(-1).year >= 1983) {
  throw new Error("Law-Dome-Rekonstruktion fehlt oder überlappt die NOAA-Beobachtung.");
}
if (payload.projectionSeries?.length !== 3 || payload.projectionSeries.some(item => item.points.length !== 4)) {
  throw new Error("Die drei Methan-Projektionssegmente fehlen.");
}
const ssp370 = payload.projectionSeries.find(item => item.scenario === "SSP3-7.0");
if (ssp370?.points.at(-1)?.value !== 3372.2) throw new Error("SSP3-7.0 wurde nicht korrekt aus Tabelle 5 übernommen.");
...
if (!curve || curve.curveRole !== "deep_dive") throw new Error("Methankurve fehlt im BLC-Export.");
...
console.log("Atmosphärisches Methan gültig: GWL ohne Kurve; BLC mit Law-Dome-Rekonstruktion, NOAA-Beobachtung und drei SSP-Projektionen.");
