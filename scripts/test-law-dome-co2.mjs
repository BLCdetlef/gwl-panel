...
if (series.provenance?.sourceFile !== "co2_annmean_gl.txt" || !series.provenance?.locator?.includes("Zeile 39") || !series.provenance?.fields?.some(field => field.startsWith("mean"))) throw new Error("Fundstelle der NOAA-Messwerte fehlt.");
...
if (historical.provenance?.sourceFile !== "law2018splines-noaa.txt" || !historical.provenance?.locator?.includes("CO2spl")) throw new Error("Fundstelle der Law-Dome-Rekonstruktion fehlt.");
...
if (exportedCo2.observationProvenance?.sourceFile !== "co2_annmean_gl.txt") throw new Error("NOAA-Fundstelle wurde nicht exportiert.");
if (!exportedCo2.historicalReconstruction[0]?.provenance?.locator?.includes("CO2spl")) throw new Error("Law-Dome-Fundstelle wurde nicht exportiert.");
if (exportedCo2.projections.some(projection => !projection.provenance?.locator?.includes(projection.scenario))) throw new Error("Szenariospalte der IPCC-Modellierung fehlt im Export.");
